...
from models.networks import SAMGraspFusion, QNetwork, Policy
...
class SAMG(object):
    ...
    def update_parameters(self, memory, batch_size, updates):
        ...
        sam_batch, grasps_batch, action_batch, \
            reward_batch, mask_batch, next_sam_batch, next_grasps_batch = memory.sample(batch_size=batch_size)

        
        # print(bboxes_batch.shape, 'sample')
        # quit(0)
        ...
        sam_batch = torch.FloatTensor(sam_batch).to(self.device)
        next_sam_batch = torch.FloatTensor(next_sam_batch).to(self.device)
