...
class CLIPGraspFusion(nn.Module):
    ...
    def forward(self, bboxes, pos_bboxes, text, actions):
        ...
        # print(bbox_feat.shape, 'bbox_feat')
        # encode text
        ...
        # print(actions.shape, grasp_feat.shape, 'grasp_feat 2')
        # quit(0)
...
# class MasksDepthEncoder(nn.Module):
#     def __init__(self, mask_generator, device):
#         super().__init__()
...
#         depth_img = torch.as_tensor(depth_img, dtype=torch.float32).unsqueeze(0).to(self.device)
#         masks = self.mask_generator.generate(color_img)
#         segs = []
#         for mask in masks:
...
#         segs = torch.as_tensor(segs, dtype=torch.float32).to(self.device)
        
#         # downsample the segmentation masks and height map from 1*224*224 to 1*512
#         segs_hmap = torch.cat((segs, depth_img), dim=0)
#         features = self.resnet(segs_hmap.unsqueeze(1).repeat(1, 3, 1, 1))
#         features = self.lstm(features.unsqueeze(0))

#         return features


class SAMGraspFusion(nn.Module):
    def __init__(self, 
                 grasp_dim=7,
                 width=512, 
                 device='cuda:0'):
        ...
        self.grasp_embbedding.to(self.device)

        # # generate logits for grasp
        # self.grasp_logits = nn.Sequential(
        #     nn.Linear(width, 256),
        #     nn.ReLU(),
        #     nn.LSTM(input_size=256, hidden_size=256, num_layers=2, batch_first=True, bidirectional=True),
        #     extract_tensor(),
        #     nn.Linear(512, 1)
        #     )
        class extract_tensor(nn.Module):
