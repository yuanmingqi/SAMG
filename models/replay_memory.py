...
class SAMGReplayMemory:
    def __init__(self, capacity, seed):
        ...
        # self.lang_buffer = []
        self.sam_buffer = []
        self.next_sam_buffer = []


    # def push(self, bbox, pos_bbox, grasp, lang_goal, action, reward, next_bbox, next_pos_bbox, next_grasp, done):
    def push(self, sam, grasp, action, reward, next_sam, next_grasp, done):
        if len(self.grasps_buffer) < self.capacity:
            # self.bboxes_buffer.append(None)
            # self.pos_bboxes_buffer.append(None)
            self.grasps_buffer.append(None)
            # self.next_bboxes_buffer.append(None)
            ...
            self.done_buffer.append(None)
            # self.lang_buffer.append(None)
            self.sam_buffer.append(None)
            self.next_sam_buffer.append(None)
        ...
        self.sam_buffer[self.position] = sam[np.newaxis, :]
        self.next_sam_buffer[self.position] = next_sam[np.newaxis, :]


        self.position = (self.position + 1) % self.capacity

    def sample(self, batch_size):
        batch = np.random.choice(range(len(self.grasps_buffer)), batch_size) 
        # lang_batch = itemgetter(*batch)(self.lang_buffer)
        # bboxes_batch = itemgetter(*batch)(self.bboxes_buffer)
        # pos_bboxes_batch = itemgetter(*batch)(self.pos_bboxes_buffer)
        grasps_batch = itemgetter(*batch)(self.grasps_buffer)
        ...
        sam = itemgetter(*batch)(self.sam_buffer)
        next_sam = itemgetter(*batch)(self.next_sam_buffer)
        
        return sam, grasps_batch, action_batch, reward_batch, done_batch, next_sam, next_grasps_batch

    def __len__(self):
        return len(self.grasps_buffer)
