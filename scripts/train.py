    ...
    for episode in range(num_episode):
        ...
        while not done:
            ...
            if episode_steps == 0:
                ...
                # cv2.imwrite("assets/color_image.png", color_image)
                # cv2.imwrite("assets/depth_image.png", depth_image)
                # cv2.imwrite("assets/mask_image.png", mask_image)
                # quit(0)
                bbox_images, bbox_positions = utils.get_true_bboxs(env, color_image, depth_image, mask_image)
                # print("Number of bounding boxes", len(bbox_images))
                # cv2.imwrite("assets/bbox_image.png", bbox_images[0])
                # print("Bounding box position", bbox_positions[0])
                # quit(0)
