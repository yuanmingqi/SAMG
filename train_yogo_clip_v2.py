...
class YOGO(nn.Module):
    ...
    def get_embeds(self, scene_image, success_grasp, failure_grasp):
        scene_features = self.scene_encoder(scene_image)
        success_grasp = th.cat([success_grasp, th.sin(success_grasp), th.cos(success_grasp)], dim=1)
        failure_grasp = th.cat([failure_grasp, th.sin(failure_grasp), th.cos(failure_grasp)], dim=1)
        success_sg_features = self.sg_encoder(success_grasp)
        success_fg_features = self.fg_encoder(success_grasp)
        failure_sg_features = self.sg_encoder(failure_grasp)
        failure_fg_features = self.fg_encoder(failure_grasp)

        return scene_features, success_sg_features, success_fg_features, failure_sg_features, failure_fg_features

    def sym_clip_loss(self, scene_features, sg_features, fg_features):
        sg_scene = self.clip_loss(scene_features, sg_features)
        fg_scene = self.clip_loss(scene_features.detach(), fg_features)
...
def build_loader(data, batch_size, device, num_workers=4):
    ...
    eval_loader = DataLoader(eval_dataset, batch_size=batch_size, shuffle=False, pin_memory=True)#, num_workers=num_workers)
...
def evaluate(model, eval_loader, device):
    ...
    with th.no_grad():
        for scene_images, success_grasps, failure_grasps in eval_loader:
            scene_features, success_sg_features, success_fg_features, failure_sg_features, failure_fg_features = \
                model.module.get_embeds(scene_images.squeeze(0).to(device), 
                      success_grasps.squeeze(0).to(device), 
                      failure_grasps.squeeze(0).to(device))
            # get scores
            sg_pos_scores = th.cosine_similarity(scene_features, success_sg_features)
            sg_neg_scores = th.cosine_similarity(scene_features, failure_sg_features)
            fg_pos_scores = th.cosine_similarity(scene_features, success_fg_features)
            fg_neg_scores = th.cosine_similarity(scene_features, failure_fg_features)

            # get accuracy
            sg_correct += (sg_pos_scores > sg_neg_scores).sum().item()
            fg_correct += (fg_pos_scores < fg_neg_scores).sum().item()

            total += scene_images.size(0)
            
    sg_accuracy = sg_correct / total
    fg_accuracy = fg_correct / total

    model.train()
    return sg_accuracy, fg_accuracy, (sg_accuracy + fg_accuracy) / 2
...
def main():
    ...
    for epoch in range(num_epochs):
        ...
        sg_acc, fg_acc, total_acc = evaluate(yogo, eval_loader, device)
        # update lr
        # scheduler.step()
