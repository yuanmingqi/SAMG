...
class YOGO(nn.Module):
    ...
    def sym_clip_loss(self, scene_features, sg_features, fg_features):
        ...
        sg_features = F.normalize(sg_features, dim=1)
        fg_features = F.normalize(fg_features, dim=1)

        # Compute similarity matrix
        logits_per_img_sg = th.matmul(scene_features, sg_features.t()) / self.temperature
        logits_per_sg = logits_per_img_sg.t()
        logits_per_img_fg = th.matmul(scene_features, fg_features.t()) / self.temperature
        logits_per_fg = logits_per_img_fg.t()

        # Labels are just the indices of the diagonal
        labels = th.arange(logits_per_sg.size(0), device=scene_features.device)

        # compute cross entropy loss for sg
        loss_img_to_sg = F.cross_entropy(logits_per_img_sg, labels)
        loss_sg_to_img = F.cross_entropy(logits_per_sg, labels)
        # compute -log softmax loss for fg
        loss_img_to_fg = F.cross_entropy(-logits_per_img_fg, labels)
        loss_fg_to_img = F.cross_entropy(-logits_per_img_fg.t(), labels)

        # Total loss is the average of the two losses
        total_loss = (loss_img_to_sg + loss_sg_to_img + loss_img_to_fg + loss_fg_to_img) / 4
...
def main():
    ...
    for epoch in range(num_epochs):
        ...
        acc = evaluate(yogo, eval_loader, device)
        # update lr
        # scheduler.step()
        print(f"E {epoch+1}/{num_epochs}, L: {total_loss:.4f}, Acc: {acc:.4f}, T: {t_e - t_s:.2f}s")
