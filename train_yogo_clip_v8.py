...
def build_loader(train_data, eval_data, batch_size, device, num_workers=4, eval=False):
    if eval:
...
def main():
    ...
    weight_decay = 0.2
    # load data    
    train_data = load_h5_file(f"datasets/single/clip_dataset_100k_train.h5")
    eval_data = load_h5_file(f"datasets/single/clip_dataset_100k_eval.h5")
    # build dataloaders
    ...
    for epoch in range(num_epochs):
        ...
        if accuracy > bench:
            ...
            if os.path.exists(f"./logs/yogo_clip_{bench:.4f}_v4.pth"):
                os.remove(f"./logs/yogo_clip_{bench:.4f}_v4.pth")
            bench = accuracy
            th.save(yogo.state_dict(), f"./logs/yogo_clip_{bench:.4f}_v4.pth")
