...
def evaluate(model, eval_loader, device):
    ...
    with th.no_grad():
        for scene_images, success_grasps, failure_grasps in eval_loader:
            ...
            pos_features = th.mean(pos_features.view(sg_len, 20, 512), dim=1)
            neg_features = th.mean(neg_features.view(sg_len, 20, 512), dim=1)
            # get scores
...
def main():
    ...
    weight_decay = 1e-4
    # load data    
    data = load_h5_file("datasets/single/clip_dataset_patch.h5")
    # build dataloaders
    ...
    for epoch in range(num_epochs):
        ...
        for scene_images, success_grasps, failure_grasps in train_loader:
            ...
            pos_features = th.mean(pos_features.view(sg_len, 20, grasp_embed_dim), dim=1)
            neg_features = th.mean(neg_features.view(sg_len, 20, grasp_embed_dim), dim=1)
